...
//  }
  ...
  if($fileType == "csv") {
    ...
    if (($file = fopen($_FILES["fileToUpload"]["tmp_name"], "r")))
    {
      ...
      if ($example_file = fopen('example_files/IXN_Test_Example.csv', 'r'))
      {
    ...
    fclose($file);
      ...
      fwrite($errorfile, 'Sorry, we were unable to open this file. \n');
  }
    } else {
  ...
  // if everything is ok, try to upload file
    ...
    $new_main_file = $target_dir . 'IXN_Projects.csv';

    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file))
    {
      // echo "The file ". htmlspecialchars( basename( $_FILES["fileToUpload"]["name"])). " has been uploaded. <br>";
      rename($target_file, $new_target_file);
      copy($new_target_file, $new_main_file);
...
echo "<script>window.location = './Admin_Home.php?upload=".$check."#Projects'</script>";
?>
